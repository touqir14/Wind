...
#define TICK_INTERVAL 10000 // 1000 milliseconds is default
#define MAX_EVENTS 10000 // 1000 is default

Scheduler::Scheduler(): max_events(MAX_EVENTS), events(new epoll_event[MAX_EVENTS]),
...
bool Task::async_read(ResizeableBuffer& in,ssize_t& read,ssize_t max) {
	read = 0;
	for(;;) {
		const ssize_t remaining = max? max-read: 512; // NUM
		if(!remaining)
			return false;
